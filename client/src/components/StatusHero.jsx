...
export default function StatusHero() {
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            ...
            const randomMetrics = [
                {
                    name: 'CPU Load', value: `${Math.floor(65 + Math.random() * 30)}%`,
                    status: Math.random() > 0.7 ? 'warning' : Math.random() > 0.9 ? 'critical' : 'normal'
                },
                {
                    name: 'Memory', value: `${Math.floor(60 + Math.random() * 25)}%`,
                    status: Math.random() > 0.8 ? 'warning' : 'normal'
                },
                {
                    name: 'Response Time', value: `${Math.floor(180 + Math.random() * 150)}ms`,
                    status: Math.random() > 0.7 ? 'critical' : Math.random() > 0.5 ? 'warning' : 'normal'
                },
                {
                    name: 'Disk I/O', value: `${Math.floor(20 + Math.random() * 20)}MB/s`,
                    status: Math.random() > 0.9 ? 'warning' : 'normal'
                }
            ];

            const probability = Math.min(0.95, Math.max(0.05, 0.7 + (Math.random() - 0.5) * 0.4));
            let status = probability > 0.8 ? 'Online' : probability > 0.5 ? 'Warning' : 'Critical';
    ...
    return (
        <section
            id="status"
            className="pt-24 pb-16 px-4 bg-black"
        >            
                        ...
                        <div className="bg-[#141414] rounded-xl shadow-xl p-6 border border-[#2A2A2A]">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center">
                                    <div className={`h-5 w-5 rounded-full ${statusColors.bg} mr-3 animate-pulse`}></div>
                                    <h3 className="text-2xl font-bold">{serverStatus.status}</h3>
                                </div>
                            ...
                            {/* Alert Message */}
                            <div className={`p-4 rounded-lg ${statusColors.light} mb-6 border ${statusColors.border} border-opacity-30`}>
                                <div className="flex">
                                    ...
                                    <div className="ml-3">
                                        <h3 className={`text-sm font-medium ${statusColors.text}`}>
                                            {serverStatus.status === 'Online'
                            ...
                            <div className="flex flex-wrap gap-4">
                                ...
                                <button className={`border ${statusColors.border} ${statusColors.text} px-4 py-2 rounded-lg transition-colors flex items-center hover:bg-opacity-10 hover:${statusColors.light}`}>
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        <div className="bg-[#141414] rounded-xl shadow-xl p-6 border border-[#2A2A2A]">
                            <div className="space-y-6">
                                {serverStatus.metrics.map((metric, index) => (
                                    <div key={index} className="flex items-center justify-between">
                                        <div className="flex items-center">
                                            <div className={`h-3 w-3 rounded-full ${getMetricColor(metric.status)} mr-3`}></div>
                                            <span className="text-gray-300">{metric.name}</span>
                                        </div>
                                        <span className={`font-mono font-bold ${getMetricColor(metric.status)}`}>
                                            {metric.value}
                                        </span>
                                    </div>
                                ))}
                            ...
                            <div className="mt-8 pt-6 border-t border-[#2A2A2A]">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <div className={`px-2 py-1 rounded ${statusColors.light} ${statusColors.text} text-xs font-medium`}>
                                        {serverStatus.status === 'Online'
                                            ? 'Stable'
                                            : serverStatus.status === 'Warning'
                                                ? 'Monitor'
